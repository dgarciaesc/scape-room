/* ============================================================
   ...
   RUTAS:
     ...
     POST /api/bundle-checkout    → crea una sesión de pago del pack (esta
                                    aventura + Tabernas con Historia)
   ...
   BINDINGS NECESARIOS:
     ...
     OTHER_DB → la base de datos D1 de Tabernas con Historia (el otro juego).
                Solo se usa para el pack de las 2 aventuras: al comprarlo,
                este Worker escribe también el código de Tabernas ahí. Sin
                este binding, el botón de "comprar las 2" simplemente no
                debe mostrarse (o fallaría el pack, aunque la compra normal
...
function generateCode(prefix = "MADRID") {
...
/* POST /api/bundle-checkout — igual que /api/checkout, pero para el
   pack de las 2 aventuras (SIGLODEORO_OTHER_GAME). Se marca la sesión
   con metadata[type]=bundle para que el webhook sepa que debe generar
   dos códigos en lugar de uno, y se avisa a gracias.html vía ?bundle=1
   de que debe esperar dos códigos, no uno. */
...
async function handleWebhook(request, env) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    const code = generateCode("MADRID");
    ...
    if (session.metadata?.type === "bundle" && env.OTHER_DB) {
      const otherCode = generateCode("TABERNAS");
...
/* POST /api/events/track — analítica: guarda un evento puntual de un
   ...
   No hay panel propio: se consulta directamente por SQL en D1, p.ej.
     SELECT * FROM events WHERE code = 'MADRID-XXXXXX' ORDER BY created_at; */
